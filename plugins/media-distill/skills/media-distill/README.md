...
- Python 3.9+
- A `.venv` virtual environment (or system Python)
...
```bash
# from your claude-media-skills directory
claude plugin install ./plugins/media-distill
```

### 2. Install Python dependencies

```bash
cd plugins/media-distill/skills/media-distill/scripts
...
Edit `scripts/.env` and add your Google Books API key to improve book search quality:
...
```bash
.venv/bin/python3 scripts/youtube_export.py <url> --lang en,ru
```
...
Channel info is cached in `scripts/.cache/cache.sqlite` with a 30-day TTL. Delete the file to force a refresh.
